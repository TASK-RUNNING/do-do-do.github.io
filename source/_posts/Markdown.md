...
2. 缺点
    * 标准不统一
        > 目前Markdown语法众多、编辑器层出不穷，每个编辑器支持的语法也不通，这就导致在一个编辑器上写的md文件，放到其它编辑器上，出现排版混乱，甚至是不支持某些语法的现象。这就是人们所说的 “**方言现象**”，这也是目前Markdown最突出的问题。
    * 图片支持不够
...
注意：不能直接写成www.baidu.com，需要加 `https://或http://`
...
一般情况下，我们插入的图片大小都不一样，如果像上面的图片一样过大，就会铺满整个屏幕，显示效果就会大打折扣。可惜Markdown没有能处理图片大小和左右对齐的语法（Markdown只注重文字，不注重精细的排版）。天无绝人之路，好在Markdown支持内嵌HTML语句，可以使用HTML来实现图片大小和对齐的控制。语法如下：
...
# 三、支持Markdown语法的软件或网站

编辑软件：
1. atom ——21世纪黑客文本编辑器，各种插件可用，也是我目前在用的编辑软件。
